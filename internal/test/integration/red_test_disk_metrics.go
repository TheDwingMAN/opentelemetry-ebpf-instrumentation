	...
	for _, path := range diskExportPaths {
		require.EventuallyWithT(t, func(ct *assert.CollectT) {
			results, err := pq.Query(
				fmt.Sprintf(`obi_stat_disk_io_latency_seconds_count{exported=%q}`, path))
...
// testDiskMetricsIOLatency asserts that the `storage_block` feature emits the
// `obi_stat_disk_io_latency_seconds` histogram with a positive `_count`, that
...
func testDiskMetricsIOLatency(t *testing.T) {
	pq := promtest.Client{HostPort: prometheusHostPort}
	for _, path := range diskExportPaths {
...
// testDiskMetricsIOLatencyWrite asserts specifically on the write operation.
...
func testDiskMetricsIOLatencyWrite(t *testing.T) {
	pq := promtest.Client{HostPort: prometheusHostPort}
	for _, path := range diskExportPaths {
		t.Run(path, func(t *testing.T) {
			require.EventuallyWithT(t, func(ct *assert.CollectT) {
				results, err := pq.Query(fmt.Sprintf(
					`obi_stat_disk_io_latency_seconds_count{exported=%q,disk_io_direction="write"}`, path))
				require.NoError(ct, err)
				enoughPromResults(ct, results)
				assert.Positive(ct, totalPromCount(ct, results))
...
func testDiskMetricsPromExposition(t *testing.T) {
	require.EventuallyWithT(t, func(ct *assert.CollectT) {
		...
		assert.Contains(ct, exposition, "# TYPE obi_stat_disk_io_latency_seconds histogram")
